-- ─────────────────────────────────────────────────────────────
-- 013_guru_net_value.sql
-- Objetivo: Capturar o Faturamento Lǭquido (Net Value) da Guru
-- ─────────────────────────────────────────────────────────────

-- 1. Adicionar coluna na tabela de eventos
ALTER TABLE public.lt_events ADD COLUMN IF NOT EXISTS net_value numeric(10,2);

-- 2. Atualizar Webhook para capturar o valor lǭquido
...
BEGIN
  ...
  -- Valor Lǭquido (net_value) - Guru envia no objeto payment ou affiliations
  v_net_val := (payment->>'net_value')::numeric;
  IF v_net_val IS NULL THEN
     -- Tenta pegar do primeiro item de affiliations se existir
     v_net_val := (affiliations->0->>'net_value')::numeric;
  END IF;
  v_net_val := COALESCE(v_net_val, v_val); -- Fallback para bruto se nǜo vier lǭquido
  ...
  IF v_lead_id IS NULL THEN
    INSERT INTO public.lt_leads (
      ...
      COALESCE(source->>'utm_source', source->>'src'),
      source->>'utm_medium', source->>'utm_campaign', source->>'utm_content', source->>'utm_term',
      COALESCE(source->>'utm_source', source->>'sck')
  ...
  ELSE
    UPDATE public.lt_leads SET 
      ...
      first_utm_source = COALESCE(first_utm_source, source->>'utm_source', source->>'src'),
      first_utm_medium = COALESCE(first_utm_medium, source->>'utm_medium'),
      first_utm_campaign = COALESCE(first_utm_campaign, source->>'utm_campaign'),
      first_utm_content = COALESCE(first_utm_content, source->>'utm_content'),
      first_utm_term = COALESCE(first_utm_term, source->>'utm_term'),
      first_page = COALESCE(first_page, source->>'sck')
    WHERE id = v_lead_id;
  END IF;

  INSERT INTO public.lt_events (
    company_id, lead_id, event_type, event_name, event_value, net_value, metadata, event_id
  ) VALUES (
    v_comp_id, v_lead_id,
    CASE WHEN v_status IN ('approved', 'paid') THEN 'purchase' ELSE 'begin_checkout' END,
    'guru_' || v_status, v_val::text, v_net_val,
    jsonb_build_object('product', v_p_name, 'source', 'guru'),
    'guru_' || v_id
  );
