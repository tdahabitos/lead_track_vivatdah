-- ─────────────────────────────────────────────────────────────
-- 012_guru_v4_tracking_fix.sql
-- Objetivo: Garantir que TODAS as UTMs (Content, Term, Campaign) sejam salvas no Update
-- ─────────────────────────────────────────────────────────────
...
BEGIN
  ...
  IF v_lead_id IS NULL THEN
    INSERT INTO public.lt_leads (
      ...
      COALESCE(source->>'utm_source', source->>'src'), -- Mapeia SRC para Source se UTM estiver vazio
      source->>'utm_medium', 
      source->>'utm_campaign', 
      source->>'utm_content', 
      source->>'utm_term',
      COALESCE(source->>'utm_source', source->>'sck') -- Mapeia SCK para Page se for a origem
  ...
  ELSE
    UPDATE public.lt_leads SET 
      ...
      -- Atualiza UTMs se estiverem vazias (First Click Attribution)
